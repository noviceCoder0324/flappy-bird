...
// physics
const velocityX = -2; //pipes moving left
let velocityY = 0; //bird jum speed
const gravity = 0.4;
...
function update() {
  ...
  //bird
  velocityY += gravity;
  bird.y = Math.max(bird.y + velocityY, 0); //apply gravity to current bird.y; limit the bird.y to the top
  context.drawImage(birdImg, bird.x, bird.y, bird.width, bird.height);
...
function moveBird(e) {
  if (e.code === "Space" || e.code === "ArrayUp") {
    velocityY = -6;
